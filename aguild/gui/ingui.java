...
public class ingui extends GuiScreen {
   private GuiTextField textbox;
   EntityPlayer player = null;
   private static final ResourceLocation tex = new ResourceLocation("aguild:textures/gui/ingui.png");

   public ingui(EntityPlayer player) {
      this.player = player;
   }
   ...
   public void func_73863_a(int par1, int par2, float par3) {
      ...
      this.field_146292_n.add(new GuiButton(0, this.field_146294_l / 2 - 30, this.field_146295_m / 2 - 15, 60, 20, "§fเชิญ"));
   }
   ...
   protected void func_146284_a(GuiButton button) {
      ...
      if (button.field_146127_k == 0 && text.length() > 3) {
         EntityPlayer target = getServerEntityByNames(this.player.field_70170_p, text);
         if (target != null) {
            mod.network.sendToServer(new packetclick(2, text, target.func_145782_y()));
         } else {
            this.player.func_145747_a(new ChatComponentText("§cไม่่มีผู้เล่นชื่อนี้อยู่ใกล้ๆ"));
         }
   ...
   public static EntityPlayer getServerEntityByNames(World world, String name) {
      ...
      do {
         ...
         o = var2.next();
      } while(!(o instanceof EntityPlayer) || !((EntityPlayer)o).func_70005_c_().equalsIgnoreCase(name));
