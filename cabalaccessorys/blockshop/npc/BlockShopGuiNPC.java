...
public class BlockShopGuiNPC extends GuiContainer {
   EntityPlayer Player = null;
   npcentity Te = null;
   private float xSize_lo;
   private float ySize_lo;
   private static final ResourceLocation iconLocation = new ResourceLocation("cabalaccessorys:textures/GuiSHOPOP.png");
   private final BlockShopInventoryNPC inventory;

   public BlockShopGuiNPC(EntityPlayer player, npcentity te, InventoryPlayer inventoryPlayer, BlockShopInventoryNPC inventoryCustom) {
      super(new BlockShopContainerNPC(player, inventoryPlayer, inventoryCustom));
   ...
   protected void func_146976_a(float f, int mouseX, int mouseY) {
      ...
      this.field_146289_q.func_78276_b("  §f" + px.npcmoney, this.field_147003_i + 116 - 60, this.field_147009_r + 66, 16777215);
   }

   protected void func_146284_a(GuiButton button) {
      EntityPlayer p = this.Player;
      PlayerData px = PlayerData.get(p);
      ItemStack sell1 = px.TeSop.SW.func_70301_a(0);
      ItemStack sell2 = px.TeSop.SW.func_70301_a(1);
      ItemStack sell3 = px.TeSop.SW.func_70301_a(2);
      ItemStack sell4 = px.TeSop.SW.func_70301_a(3);
      ItemStack sell5 = px.TeSop.SW.func_70301_a(4);
      ItemStack sell6 = px.TeSop.SW.func_70301_a(5);
      if (button.field_146127_k == 0) {
         ...
         if (sell6 != null) {
            px.setprice = 6;
            p.openGui(Start.instance, Start.SGBOP, p.field_70170_p, (int)p.field_70165_t, (int)p.field_70163_u, (int)p.field_70161_v);
         } else {
            p.func_145747_a(new ChatComponentText("§f[§aระบบ§f] §cช่องที่ 6 ไม่มีของที่จะขาย"));
         }
      } else if (button.field_146127_k == 3) {
         Start.networkChannel.sendToServer(new BlockOpenShopNewPacketNPC(0));
         p.func_71053_j();
      } else if (button.field_146127_k == 7) {
         Start.networkChannel.sendToServer(new BlockOpenShopNewPacketNPC(2));
         p.func_71053_j();
      } else if (button.field_146127_k == 8) {
         Start.networkChannel.sendToServer(new BlockOpenShopNewPacketNPC(1));
         p.func_71053_j();
      }
