...
public class BlockGuiPrice extends GuiScreen {
   private GuiTextField textbox;
   EntityPlayer Player = null;
   int price;
   public int sprice;

   public BlockGuiPrice(EntityPlayer player, int setprice) {
      this.Player = player;
   ...
   protected void func_146284_a(GuiButton button) {
      if (button.field_146127_k == 0) {
         ...
         this.getValue();
         if (this.price != 0) {
            Start.networkChannel.sendToServer(new BlockSetPriceNewPacket(this.price, this.sprice));
            p.func_71053_j();
