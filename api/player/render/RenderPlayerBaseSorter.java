...
public final class RenderPlayerBaseSorter {
   ...
   public RenderPlayerBaseSorter(List list, Map allBaseSuperiors, Map allBaseInferiors, String methodName) {
      this.list = list;
            ...
            for(offset = 0; offset < this.list.size() - 1; ++offset) {
               for(size = offset + 1; size < this.list.size(); ++size) {
                  ...
                  if (leftWantsToBeInferiorToRight && rightWantsToBeInferiorToLeft) {
                     throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + this.methodName + "'. '" + left + "' wants to be inferior to '" + right + "' and '" + right + "' wants to be inferior to '" + left + "'");
                  }

                  if (leftWantsToBeSuperiorToRight && rightWantsToBeSuperiorToLeft) {
                     throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + this.methodName + "'. '" + left + "' wants to be superior to '" + right + "' and '" + right + "' wants to be superior to '" + left + "'");
                  }

                  if (leftWantsToBeInferiorToRight && leftWantsToBeSuperiorToRight) {
                     throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + this.methodName + "'. '" + left + "' wants to be superior and inferior to '" + right + "'");
                  }

                  if (rightWantsToBeInferiorToLeft && rightWantsToBeSuperiorToLeft) {
                     throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + this.methodName + "'. '" + right + "' wants to be superior and inferior to '" + left + "'");
                  }
               }
            }
      ...
      if (directInferiors == null) {
         ...
         while(iter.hasNext()) {
            ...
            if (inferiorType == startType) {
               throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + this.methodName + "'. Circular superiosity found including '" + startType + "'");
            }
            ...
            try {
               ...
               throw new UnsupportedOperationException("Can not sort RenderPlayerBase classes for method '" + this.methodName + "'. Circular superiosity found including '" + inferiorType + "'", var9);
            }
